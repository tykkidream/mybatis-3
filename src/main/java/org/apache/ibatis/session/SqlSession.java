...
public interface SqlSession extends Closeable {
	 ...
	 * </p>
	 * <p>
	 * 检索一行数据的映射（查询一行数据并映射成 JavaBean 对象），它根据语句的Key（映射文件中的 namespace + 语句的 id ）
	 * 和参数由 {@code ResultHandler} 处理后获得。
	 * </p>
	 * 
	 * @param statement
	 *            <p>
	 ...
	 * </p>
	 * <p>
	 * Mapper 实例最终由 {@link MapperRegistry#getMapper(Class, SqlSession)} 创建，一般使用
	 * {@link Configuration#getMapper(Class, SqlSession)} 较简便·。 这两种方式都需要先获得
	 * {@link Configuration} 实例，要知道通过本接口的 {@link #getConfiguration()} 就可获得
	 * 。本方法相比刚才的两个方法它少一 个本接口类型的参数，因为那个参数就是自己本身，这样减少一些复杂增加一些简便。
	 * </p>
	 * <hr>
	 * <p>
	 * Retrieves a mapper.
